<template>
  <div class="limiter">
    ...
    <v-card class="filtrorelatorios">
      ...
      <v-text-field
        v-model="periodo"
        label="Informe o periodo que deseja filtrar"
        class="filtro"
        persistent-hint
        return-object
        single-line
      ></v-text-field>
      <v-btn color="green" class="botaoA" @click="exibir()"> Consultar </v-btn>

      <router-link to="./menusupervisorsainte" class="linkp">
        <v-btn dark class="botaoSair">voltar</v-btn>
      </router-link>
      <v-btn class="botaoexportar" @click="saveCSV()" style="width: 10%">
        Download<br />CSV
      </v-btn>
    </v-card>
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Nome Agente", value: "solicitantenome" },
        { text: "Data e Hora", value: "datahora" },
        { text: "Duração", value: "duracao" },
        { text: "Duração Espera", value: "holdtime" },
        { text: "Telefone", value: "teleatendente" },
  ...
  methods: {
    ...
    exibir: async function () {
      ...
      console.log("GRUPOOOOOOOOOOOOOOOOOOOOOOOOO", grupo);
      
      let a = await api.get(
        `/detalhessainte/${grupo}/${this.d1}/${this.d2}/${this.periodo}`
      );
      console.log(`/detalhessainte/${this.d1}/${this.d2}`);
      console.log(a.data.dados);
      this.dados = a.data.dados;

      console.log("to aqui", this.periodo);
      console.log("to a1321232321313123131qui");
    },

    saveCSV() {
      this.filename = "PlugPhone Detalhes";
      //================================================================
      let csvFile = `"N º do agente";"Data e Hora";"Duração";"Duração de espera";"Fila";"Telefone";"Status De Desligamento";"estado"\n`;
      this.dados.forEach((a) => {
        csvFile += `"${a.teleatendente}";"${a.nomesolicitante}";"${a.datahora}";"${a.duracao}";"${a.holdtime}";"${a.fila}";"${a.solicitante}";"${a.razao}";"${a.estado}"\n`;
      });
