...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Abandonou", value: "somaAbandon" },
        { text: "Atendidas", value: "somaComplete" },
  ...
  methods: {
    ...
    exibir: async function () {
      let getSomaEntrada = await api.get(
        `/entroucomg/${this.d1}/${this.d2}`
      );
      let somaEntradaArray = getSomaEntrada.data.dados.length;
      let somaEntrada = somaEntradaArray;


      //console.log("Soma abandon: " + somaAbandon);

      //console.log("Soma Entrada: " + somaEntrada);

      let getSomaComplete = await api.get(
        `/completecomg/${this.d1}/${this.d2}`
      );
      let somaCompleteArray = getSomaComplete.data.dados.length;
      let somaComplete = somaCompleteArray;

      //console.log("Soma Complete: " + somaComplete);

      let somaAbandon = somaComplete - somaEntrada

      if (somaAbandon < 0){
        somaAbandon *= -1
      }

      let porcento = (somaComplete / somaEntrada) * 100;
      //console.log("porcento", porcento);
      let mediaform = parseInt(porcento) + "%";
      //console.log("media form: " + mediaform);

      this.dados = [{ somaEntrada, somaComplete, somaAbandon, mediaform }];
      ...
      let atendida =  somaEntrada - somaComplete  
      //let atendidaG = parseInt(atendida);

      let abandon = somaComplete

      console.log('atendida', atendida, '| abandon', abandon)


      this.chartData[1][1] = abandon;
