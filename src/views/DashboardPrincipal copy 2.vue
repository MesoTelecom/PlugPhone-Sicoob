    ...
    <v-container>
      <v-row>
        <v-col cols="12" sm="12">
          ...
          <div style="display: flex; margin-left: -82px; margin-top: 1%">
            <v-col cols="12" sm="7">
                        <GChart
    type="LineChart"
    :data="chartData1"
...
setIntervalAsync(
  ...
  components: {
   ...
   let fluxo = await api.get(`/fluxohora/${this.PastDateTime()}`);
    //console.log(fluxo.data.dados);
    if (!fluxo.data.dados) {
      return alert("não veio nada");
    }
    if (fluxo.data.dados.length == 0) {
      return alert("Não existe ligação");
    }
    ...
    this.chartData1.push([hora, total]);
...
,
  emligacao: async function(){
    let liga = await api.get("/dashligacao")
    //console.log(liga.data.dados)
    let realliga = liga.data.dados.length
    //console.log(realliga)
    this.lists.count.push([realliga])
