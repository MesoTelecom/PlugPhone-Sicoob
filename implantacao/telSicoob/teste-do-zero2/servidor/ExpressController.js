...
class ExpressController {
  ...
  initializeRoutes() {
    ...
    this.expressAppWrapper.get('/', (req, res) => {
      res.send('API PLUGPHONE');
    });

    this.expressAppWrapper.app.use('/whatsapp', proxy('https://whatsapp.sicoob.plugphone.cloud:5995/'));
    ...
    this.expressAppWrapper.get("/buscarcontatos/:usuario/:tipo", async (req, res, next) => {
      ...
      if (tipo === 'Analista') {
        ...
        let qry = `select distinct m.credor as nome, m.Telefone, m.processo, m.estado from meso_mealing as m inner join meso_oportunidade as o on (m.idMealing = o.idMealing) where o.estado = 'aprovado';`
        console.log('contato', qry)
        let res20 = await executaQry(qry)
        res.json(res20)
        console.log('contatos resposta', res20)
    ...
    this.expressAppWrapper.post("/loginconfere", async (req, res) => {
      ...
      try {
        ...
        if (res1.dados.length > 0) {
          ...
          const idAgencia = res1.dados[0].id_agencia

          console.log('CAIDO', { token, tipo, usuario2, idPermissao, idAgencia })
          res.json({ token, tipo, usuario2, idPermissao, idAgencia });
    ...
    let inserirCsv = async (resultadoCsv, usuario) => {
      for (const e of resultadoCsv) {
        ...
        let qry = `INSERT INTO meso_mailing (nome, telefone) VALUES ('${e.nome}','${e.telefone}')`;
    ...
    */
    ...
    this.expressAppWrapper.get("/getAgencia/:id", async (req, res) => {
      let id = req.params.id;

      try {
        let qry = `
        select id_agencia from meso_usuariologin where id = '${id}'`;
        console.log('id_agencia', qry)
        let res27 = await executaQry(qry);
        res.json(res27);
      } catch (e) {
        //////console.log(e);
    ...
    this.expressAppWrapper.get('/oportunidade/:processo/:plataforma', async (req, res, next) => {
      ...
      if (isCadastrado.dados.length == 0) {
        ...
        console.log('o cadastro aqui', qry)
    ...
    this.expressAppWrapper.get('/busca-permissao/:id_usuario', async (req, res) => {
      let id_usuario = req.params.id_usuario;
      try {
        let qry = `
          SELECT p.permissao, p.nome as tipo
          FROM meso_permissao p
          JOIN meso_usuariologin up ON up.id_permissao = p.id_permissao
          WHERE p.id_permissao = '${id_usuario}'
        `;

        console.log('eu sou o busca-permissao', qry)
        ...
        let permissao = resultado.dados[0].permissao;
        let tipo = resultado.dados[0].tipo;

        res.json({ permissao, tipo });
      } catch (e) {
    ...
    this.expressAppWrapper.get('/finaliza/:processo/:estado', async (req, res, next) => {
      ...
      if (estado == 'aprovado') {
        let qry3 = `update meso_mealing set estado = 'mdi-checkbox-marked-circle-outline' where processo = '${processo}'`
        console.log('Aprovado', qry3)
        await executaQry(qry3)
      } else {
        if (estado == 'reprovado') {
          let qry4 = `update meso_mealing set estado = 'mdi-cancel' where processo = '${processo}'`
          console.log('Reprovado', qry4)
    ...
    this.expressAppWrapper.get('/usupermissao/:id', async (req, res, next) => {
      let id = req.params.id

      let qry1 = `select permissao from meso_permissao where id_permissao = '${id}'`
      console.log(qry1)
    ...
    this.expressAppWrapper.get('/tmazap/:analista/:d1/:d2', async (req, res, next) => {
      ...
      let qry1 = `select u.usuario as nome,p.platarforma as plataforma,count(o.idOportunidade)as quantidade,sec_to_time(sum(time_to_sec(timediff(o.dataFim,o.dataInicio)))) as tempo  from meso_oportunidade as o inner join meso_plataforma as p on (o.idPlataforma = p.idPlatarforma) inner join meso_usuariologin as u on (o.idAnalista = u.id) where p.idPlatarforma = 3 and u.usuario = '${analista}' and dataInicio >= '${data1}' and dataInicio <= '${data2}';`
      let res1 = await executaQry(qry1)
      let qry2 = `select u.usuario as nome,p.platarforma as plataforma,count(o.idOportunidade)as quantidade,sec_to_time(sum(time_to_sec(timediff(o.dataFim,o.dataInicio)))) as tempo from meso_oportunidade as o inner join meso_plataforma as p on (o.idPlataforma = p.idPlatarforma) inner join meso_usuariologin as u on (o.idAnalista = u.id) where p.idPlatarforma = 4 and u.usuario = '${analista}' and dataInicio >= '${data1}' and dataInicio <= '${data2}';`
    ...
    this.expressAppWrapper.get(`/adicionaContato/:nome/:telefone/:grupo`, async (req, res, next) => {
      let nome = req.params.nome;
      let telefone = req.params.telefone;
      let grupo = req.params.grupo;
      try {
        let qry = `
              insert into meso_contatos(nome, telefone, setor) values ('${nome}', '${telefone}', '${grupo}')
            `;
        console.log(qry)
    ...
    this.expressAppWrapper.get('/buscarcsv', async (req, res, next) => {
      if (terminou) {
        let qry = `select * from meso_mailing`
        console.log(qry)
        let res1 = await executaQry(qry)
        res.json(res1)
        console.log('to aqui')
    ...
    this.expressAppWrapper.get("/realoperador", async (req, res, next) => {
      // let filacompleta = req.params.fila;
      //let data1 = req.params.d1 + ' 00:00:00'
      //let data2 = req.params.d2 + ' 23:59:59'
    ...
    this.expressAppWrapper.get("/ajustaEstado", async (req, res, next) => {
      ...
      let qry = `update meso_operadores set estado = 'deslogado' where fila = 0 and estado = 'logado';`;
      console.log(qry);
    ...
    this.expressAppWrapper.post("/insereusuario", async (req, res, next) => {
      ...
      try {
        let qry = `
            ...
            `;
        console.log('Teste teste testa ai', qry)
        let res26 = await executaQry(qry);
        res.json(res26);
      } catch (e) {
        console.log(e);
    ...
    this.expressAppWrapper.get('/buscarmealing/:telefone', async (req, res, next) => {
      let telefone = req.params.telefone
      console.log("Olha Auqui", telefone)
    ...
    this.expressAppWrapper.get("/estadoMealing/:processo/:atendeu/:reagendar/:interesse/:negociar/:observacao", async (req, res, next) => {
      ...
      if (atendeu == 'sim') {
        ...
        if (interesse == 'sim' || negociar == 'sim') {
          ...
          let res1 = await executaQry(qry)
          res.json(res1)
          let qry2 = `update meso_mealing set estado = 'mdi-checkbox-marked-circle-outline' where processo = '${processo}'`
          console.log('qry2', qry2)
          ...
          console.log('query', qry)
          let qry2 = `update meso_mealing set estado = 'mdi-cancel' where processo = '${processo}'`
          console.log('qry2', qry2)
          await executaQry(qry2)
        ...
        let res1 = await executaQry(qry)
        res.json(res1)
        let qry3 = `update meso_mealing set estado = 'mdi-cancel' where processo = '${processo}'`
    ...
    this.expressAppWrapper.post('/alterargrupo', async (req, res, next) => {
      let mostra = req.body
      let { id, grupo, descricao } = mostra

      try {
        let qry = `update meso_grupo set grupo = '${grupo}', descricao = '${descricao}' where id = ${id}`
        let res14 = await executaQry(qry);
        res.json(res14);
      } catch (e) {
        res.json({ message: e.message })
    ...
    this.expressAppWrapper.post("/inseregrupo", async (req, res, next) => {
      let grupo = req.body.grupo
      let descricao = req.body.descricao
      try {
        let qry = `
          insert into meso_grupo (grupo, descricao)
    ...
    this.expressAppWrapper.get('/deletegrupo/:id', async (req, res, next) => {
      let id = req.params.id;
      try {
        let qry = `delete from meso_grupo where id = ${id}`;
        let res20 = await executaQry(qry)
        res.json(res20)
      } catch (error) {
        ////////console.log(e)
    ...
    this.expressAppWrapper.get('/buscarmensagem/:protocolo', async (req, res, next) => {
      let protocolo = req.params.protocolo
      console.log("Olha Auqui", protocolo)
      let qry = `select * from mensagens where protocolo = '${protocolo}'`
      console.log('sou qry mealing', qry)
      let res1 = await executaQry(qry)
      res.json(res1)
    })
