...
-->
      <v-select v-model="pintotal" :items="items2" item-text="nome" item-value="pin" label="Informe o nome do operador"
        class="filtro" persistent-hint return-object single-line style="right: 8%; margin-left: 20%"></v-select>
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Duração total (m)", value: "durmin" },
        { text: "TMA (m)", value: "tmamin" },
        { text: "Máxima Duração (m)", value: "durmaxmin" },
      ],
      dados: [],
      items: [],
      items2: [],
  ...
  methods: {
    ...
    listar: async function () {
      ...
      console.log('Olha aqui',listaTotalAgente)
    },
    exibir: async function () {
      ...
      let mediaTma = await api.get(`/mediatmasainte/${ramalNum}/${this.d1}/${this.d2}`)
      ...
      let totalChamadas = await api.get(
        `/totalchamadastmasainte/${this.d1}/${this.d2}/${ramalNum}`
      );

      let realTotalChamadas = totalChamadas.data.dados.length;

      let totalDuracao = await api.get(
        `/totalduracaotmasainte/${this.d1}/${this.d2}/${ramalNum}`
      );
      ...
      realTotalDuracao.forEach((d) => {
        duracao = d.duracaototal;
        duracaoMinutos = duracao
        return duracaoMinutos;
      });

      let maximaDuracao = await api.get(
        `/maximaduracaotmasainte/${this.d1}/${this.d2}/${ramalNum}`
      );
      ...
      let ramalOperador = await api.get(
        `/ramaltmasainte/${ramalNum}`
      );
      let realRamal = ramalOperador.data.dados;

      let ramalAgente;
      realRamal.forEach((d) => {
        ramalAgente = d.ramal;
        //console.log("Nome do usuário:", ramalagente);

        return ramalAgente;
      });

      let nomeOperador = await api.get(
        `/nomeramalsainte/${ramalNum}`
      );

      let realNome = nomeOperador.data.dados;
      ...
      let tmamin = tmaMinutos;
      let durmin = duracaoMinutos;
      let durmaxmin = maximaMinutos;
      this.dados = [
        {
          ramalNum,
          nome,
    ...
    saveCSV() {
      this.filename = "PlugPhone TMA";
      //================================================================
      let csvFile = `"ramal do agente";"nome do agente";"chamadas atendidas";"Duração total(m)";"TMA(m)";"Maxima Duração(m)";\n`;
      this.dados.forEach((a) => {
        csvFile += `"${a.nome}";"${a.nomeagente}";"${a.realTotalChamadas}";"${a.duracaominutos}";"${a.tmamin}";"${a.maximaminutos}"\n`;
      });
