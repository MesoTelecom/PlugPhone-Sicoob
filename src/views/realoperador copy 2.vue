...
export default {
  ...
  methods: {
    toggleEstado() {
      ...
      this.isPausado = this.textoBotao == "Iniciar" ? 0 : 1
      ...
      if (this.eFoi === true) {
        ...
        //console.log('Xuxu beleza', this.eFoi)
      } else if (this.eFoi === false) {
        clearInterval(this.idsetinterval)
        //console.log('chupeta de baleia if', this.eFoi)
      }

    },
    anterior: async function () {
      if (this.emExecucao == false && this.isPausado == 0) {
        let a = await api.get(`/realoperadorrtanterior/${this.isCarregado}`);
        ////////console.log(`/realoperadorrt`);
        ////////console.log(a.data.dados);
        this.dados = a.data.dados;
        this.isCarregado = false
      } else {
        window.alert('Por Favor Pause Antes De Ir Para O Anterior')
      }
    },
    proximo: async function () {
      if (this.emExecucao == false && this.isPausado == 0) {
        let a = await api.get(`/realoperadorrt/${this.isCarregado}`);
        ////////console.log(`/realoperadorrt`);
        ////console.log('suco de caju',a.data.dados);
        this.dados = a.data.dados;
        //////console.log('indice aqui', this.dados.length)
        this.isCarregado = false
      } else {
        window.alert('Por Favor Pause Antes De Ir Para O Proximo')
      }
    },
    ...
    /*iniciaFunc: async function(){
      this.grupo = "Grupo A"
      
      let a = await api.get(`/realoperadorrtGrupoA/${this.grupo}`);
    ...
    exibir: async function () {
      //////console.log('carregado',this.isCarregado)
      let a = await api.get(`/realoperadorrt/${this.isCarregado}`);
      if (this.executa) {
        ////////console.log(`/realoperadorrt`);
        ////////console.log(a.data.dados);
      ...
      /*else if (this.isPausado == 0) {
        if (this.dados.length == 1){
          var buscarGrupo = this.dados[0].grupo
        }else {
          buscarGrupo = 'TODOS'
        }
        let b = await api.get(`/realoperadorrtgrupo/${buscarGrupo}`);
        this.dados = b.data.dados        
        //////console.log(b)
        //////console.log('ma oi', buscarGrupo)
      }*/
      this.isCarregado = false
    },
