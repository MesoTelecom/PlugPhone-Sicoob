...
export default {
    name: "HelloView",
  async beforeMount() {
    let a = await api.get("/recebidascomsucesso");
    console.log(a);
    setInterval(() => this.exibir(), 2000);
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Tempo de espera total", value: "espera" },
        { text: "Abandonadas", value: "abandon" },
        { text: "Finalizadas", value: "sucesso" },
        { text: "Entrou", value: "join" },
      ],
  ...
  methods: {
    ...
    exibir: async function () {
      ...
      let a = await api.get(
        `/recebidafila/${this.fila}/${this.PastDateTime()}/${this.currentDateTime()}`
      );

      let recebida = a.data.dados.length;

      let b = await api.get(
        `/recebidasucesso/${this.fila}/${this.PastDateTime()}/${this.currentDateTime()}`
      );

      let sucesso = b.data.dados.length;

      let c = await api.get(
        `/recebidaabandon/${this.fila}/${this.PastDateTime()}/${this.currentDateTime()}`
      );

      let abandon = c.data.dados.length;

      let d = await api.get(
        `/recebidajoin/${this.fila}/${this.PastDateTime()}/${this.currentDateTime()}`
      );

      let join = d.data.dados.length;

      let e = await api.get(
        `/recebidaespera/${this.fila}/${this.PastDateTime()}/${this.currentDateTime()}`
      );

      let tempo = e.data.dados;

      let h = await api.get(
        `/recebidaesperaatende/${this.fila}/${this.PastDateTime()}/${this.currentDateTime()}`
      );
      ...
      this.dados = [
        { recebida, sucesso, abandon, join, espera, filaS, esperaAtende },
      ]}
  },
};
</script>
