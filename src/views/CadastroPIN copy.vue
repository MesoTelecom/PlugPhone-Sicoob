...
export default {
  ...
  methods: {
    ...
    async save() {
   //inclui
	let res = await api.post("dados/inserepin",this.insereItem);
    if (res.data.msg=="erro") {
    window.alert("Ocorreu um erro - code 2!");
	} else {
    this.desserts.push(this.insereItem);
	window.alert("Operador Cadastrado com sucesso!");
