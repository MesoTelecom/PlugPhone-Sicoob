  ...
  <div class="limiter">
    ...
    <div>
      <v-card class="filtrorelatorios">
        <v-select :items="items" label="Informar o número da fila" v-model="fila" class="filtro"></v-select>
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Aguardando na fila", value: "contafila" },
        // { text: "Bina do Aguarde", value: "tudofila" },
        { text: "Abandonadas", value: "abandon" },
        { text: "Finalizadas", value: "sucesso" },
        { text: "Totais", value: "join" },
      ],
  ...
  methods: {
    ...
    exibir: async function () {
      ...
      let c = await api.get(
        `/recebidaabandon/${this.fila
        }/${this.PastDateTime()}/${this.currentDateTime()}`
      );

      let abandon = c.data.dados.length;
      let tudoabandon = c.data.dados;
      console.log("tudoabandonado: ", tudoabandon);

      let d = await api.get(
        `/recebidajoin/${this.fila
        }/${this.PastDateTime()}/${this.currentDateTime()}`
      );
      ...
      // Fila em realtime
      let joinfila = await api.get(`/listajoin/${this.fila}`);
      // let entrajoin = join.data.dados;
      //console.log(join);
      let contafila = joinfila.data.dados.length;
      let tudofila = joinfila.data.dados;
      console.log("o que vem?", tudofila);

      let chamador;
      tudofila.forEach((d) => {
        chamador = d.calleridnum;

        return chamador;
      });
      console.log("for each chamador:", chamador);

      this.dados = [
        { recebida, sucesso, abandon, join, filaS, contafila, tudofila },
      ];
    },
  },
};
