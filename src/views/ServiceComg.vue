...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Abandonou", value: "abandon" },
        { text: "Atendidas", value: "contacomplete" },
        { text: "Service(%)", value: "mediaform" },
      ],
      dados: [],
      items: [],
      items2: [],
  ...
  methods: {
    ...
    exibir: async function () {
      let entradaGet = await api.get(
        `/entroucomgnovo/${this.d1}/${this.d2}`
      );
      let estadoGet = await api.get(
        `/estadocomgnovo/${this.d1}/${this.d2}`
      );



      let entrada = entradaGet.data.dados

      let estado = estadoGet.data.dados
      ...
      entrada.forEach((d) => {
        let numstatus = {}
        estado.forEach((c) => {
          if (d.entrou == c.solicitante) {
            numstatus = c
          }
        })
        tabela.push(numstatus)
      })

      console.log('tabela', tabela)

      let entrounovo = tabela.length

      console.log(entrounovo)

      let complete = tabela.filter(a => a.estado == 'atendida')

      console.log(complete)

      let contacomplete = complete.length
      console.log(contacomplete)

      let abandon = entrounovo - contacomplete

      console.log(abandon)

      let porcento = (contacomplete / entrounovo) * 100

      let mediaform = parseInt(porcento) + "%";

      console.log(mediaform)

      this.dados = [{ entrounovo, contacomplete, abandon, mediaform }];
      ...
      let atendida = entrounovo - contacomplete
      //let atendidaG = parseInt(atendida);

      let abandona = contacomplete

      console.log('atendida', atendida, '| abandon', abandona)


      this.chartData[1][1] = abandona;

      this.chartData[2][1] = atendida;
    },
    ...
    saveCSV() {
      ...
      this.dados.forEach((a) => {
        csvFile += `"${a.agentepin}";"${a.nomeagente}";"${a.ramal}";"${a.filaagente}";"${a.realtotalchamadas}";"${a.duracao}";"${a.dadotratado}";"${a.mediaform}"\n`;
      });
