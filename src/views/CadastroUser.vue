<template>
  <div class="limiter">
    ...
    <v-data-table :search="search" :headers="headers" :items="desserts" sort-by="calories" class="elevation-1">
      <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title>Usuários de Painel PlugPhone</v-toolbar-title>
              ...
              <v-card-text>
                <v-container>
                  <v-row>
                    ...
                    <v-col cols="12" sm="6" md="12">
                      <v-select :items="items" v-model="editedItem.tipo" label="SELECIONE" outlined></v-select>
                    </v-col>
                    <v-col class="d-flex" cols="12" sm="6">
                      <v-select :items="pegaGrupo" v-model="editedItem.tipo" label="Outlined style" outlined></v-select>
                    </v-col>
                  </v-row>
                </v-container>
              </v-card-text>
...
export default {
  ...
  methods: {
    async initialize() {
      let res = await api.get("/listausuariojson");
      console.log(res.data.dados);
      this.desserts = res.data.dados;

      let usuario = res.data.dados[0].usuario



      let tipo = res.data.dados[0].tipo
      let grupo = JSON.parse(res.data.dados[0].grupo.split(','))
    ...
    async save() {
      ...
      if (this.editedIndex > -1) {
        ...
        res = await api.post("/usuarioaltera", this.editedItem);
        if (res.data.msg == "erro") {
