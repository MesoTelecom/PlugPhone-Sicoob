...
*/
    ...
    async funcTokenFirebase() {
      ...
      let a = await apiWP.post('/registrar-token', { "usuario": nome + "-Meso", "token": token });
      console.log(a)
    },
    ...
    async sendMessage() {
      ...
      if (this.newMessage.trim() !== "") {
        ...
        console.log('verifica resposta da API', resposta)
        if (resposta == "mensagem não tolerada") {
    ...
    async buscarContato() {
      this.contacts = []
      let contatos = await apiWP.get(`/buscarcontatos/${this.tipo}`);
