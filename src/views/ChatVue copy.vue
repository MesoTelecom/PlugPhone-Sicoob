    ...
    <v-app style="">
      ...
      <v-navigation-drawer app color="rgb(221 221 221)" class="sidebar">
        ...
        <v-row>
          ...
          <v-icon @click="openDialogFiltrado = true" class="imageIcon" style="left: 76%;font-size: 169%;">
            mdi-cog
          </v-icon>
      ...
      <v-main style="padding: 0px; height: 100vh; display: flex; flex-direction: column;">
        <v-container fluid style="flex: 1; display: flex; flex-direction: column; overflow: hidden;">
          <v-row class="cabecalho">
            <v-btn @click="buscarContato('Geral', estadoContatoFiltro = 'Todos')" class="botaoEstado">Todos</v-btn>
            <v-btn @click="buscarContato('Geral', estadoContatoFiltro = 'Novo')" class="botaoEstado">Novo</v-btn>
            <v-btn @click="buscarContato('Geral', estadoContatoFiltro = 'Aguardando Cliente')"
              class="botaoEstado">Aguard... Cliente</v-btn>
            <v-btn @click="buscarContato('Geral', estadoContatoFiltro = 'Aguardando Atendimento')"
              class="botaoEstado">Aguard... Atendimento</v-btn>
            <v-btn @click="buscarContato('Geral', estadoContatoFiltro = 'Concluido')"
              class="botaoEstado">Concluido</v-btn>
          </v-row>

          <v-row style="flex: 1; overflow-y: auto; margin-right: 25%;" ref="messages">
    ...
    <div class="bottom-bar" style="width: 73%;padding-top: 5%;position: sticky;bottom: 0%;">
      ...
      <v-dialog v-model="openDialogFiltrado" max-width="500px" persistent>
        <v-card class="dialogoZap">
          ...
          <v-card-text>
            <!-- TextField no topo -->
            <v-text-field style="width: 100% !important;" label="Digite o valor do filtro" outlined
              v-model="filtroValor" class="mb-4"></v-text-field>

            <!-- Radio Group para seleção única -->
            <v-radio-group v-model="filtroSelecionado">
              <v-radio @click="buscarContato('Nome', estadoContatoFiltro)" label="Nome" value="nome"></v-radio>
              <v-radio @click="buscarContato('Cpf', estadoContatoFiltro)" label="CPF" value="cpf"></v-radio>
              <v-radio @click="buscarContato('Geral', estadoContatoFiltro)" label="Geral" value="geral"></v-radio>
            </v-radio-group>
          </v-card-text>

          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="primary" text @click="openDialogFiltrado = false">Concluir</v-btn>
            <v-btn color="secondary" text @click="openDialogFiltrado = false">Cancelar</v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
...
export default {
  ...
  async beforeMount() {
    ...
    this.idsetinterval = setInterval(() => this.buscarContato(this.filtroCargo, this.estadoContatoFiltro), 5000);
  ...
  methods: {
    async buscaProtocolo(telefone) {
      ...
      protocolo.forEach(e => {
        this.protocoloArray.push(e.protocolo + " " + e.datetime)
      });
    ...
    async finalizar(finaliza) {
      if (finaliza == true) {
        apiWP.get(`/finalizaprotocolo/${this.wppnum}`)
        apiWP.get(`/updateprotocolo`)
      }
    },
    ...
    async populaOportunidade(plataforma) {
      ...
      /*if (this.tipo == 'Analista') {
        ...
        let template = await apiWP.post("/sendtemplate", msg); descomentar aqui
    ...
    async sendMessage() {
      ...
      if (this.newMessage.trim() !== "") {
        ...
        let resposta = await apiWP.post("/send", msg);
    ...
    async uploadDocumento() {
      ...
      try {
        ...
        this.messages.push({
          text: `https://whatsapp.sicoob.plugphone.cloud:5995/midia/${caminhoLimpo}`,
    ...
    async buscarContato(filtro, estadoContato) {
      ...
      if (filtro == "" || this.filtroValor == "") {
        ...
        contatos = await apiWP.get(`/buscarcontatos/${this.tipo}/${filtro}/${this.filtroValor}/${estadoContato}`);
