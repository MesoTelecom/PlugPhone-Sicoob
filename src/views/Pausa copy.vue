  ...
  methods: {
    ...
    exibir: async function () {
      let confereramal = await api.get(
        `/realoperadorpausado/${this.pintotal.pin}/${this.d1}/${this.d2}/${this.fila.extension}`
      );
      let ramaldopin = confereramal.data.dados;
      //Aqui eu to pegando os dados inteiros da tabela logs. Todas as pausas, login e logout
      console.log("eu mostro o ramaldopin", ramaldopin);

      console.log("json da api", ramaldopin);
      let ramal = [];
      let acao = [];
      let fila = [];
      let nomeagente = [];
      ...
      console.log("Eu sou as datas separadas: ", t1, t2, ramal, acao, fila);

      let sodata1 = t1.substr(0, 10);

      let sodata2 = t2.substr(0, 10);

      let verduracao = await api.get(
        `/duracpausa/${sodata1}/${sodata2}/${ramalnum}/${this.fila.extension}`
      );
      let forverduracao = verduracao.data.dados;
      let duracao = [];

      forverduracao.forEach((d) => {
        duracao = d.duracao;
      ...
      this.dados = [{ ramal, nomeagente, acao, fila, pin, duracao }];
