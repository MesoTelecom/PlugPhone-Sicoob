...
const { send, sendImage, sendVideo, sendAudio, sendDocument, sendTemplate, download, test } = require('/whatsapp/webhook/methods.js');
...
class ExpressController {
  ...
  initializeRoutes() {
    ...
    // Middleware de upload com limites de tamanho
    const upload =  multer({
      storage,
    ...
    this.expressAppWrapper.post('/upload',  upload.single('image'), async (req, res) => {
    ...
    // Rota para upload via URL com Axios
    this.expressAppWrapper.post('/axios-upload', (req, res) => {
      const imageUrl = req.body.imageUrl; // URL da imagem enviada
      ...
      axios
        ...
        .then((response) => {
          ...
          fs.writeFile(filepath, imageBuffer, (err) => {
            ...
            res.status(200).send(`Imagem salva: ${filename}`);
          });
        })
        .catch((error) => {
...
//    const upload = uploadArquivo();
    ...
    */
    ...
    this.expressAppWrapper.listen(this.porta, () => console.log('serv perere ' + this.porta))
  }
}
